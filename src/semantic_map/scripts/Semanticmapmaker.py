...
#         self.cam_sub = message_filters.Subscriber('/keyframe_pose', PoseStamped)
 #        self.cam_sub = message_filters.Subscriber('/orb_pose', PoseStamped)
         self.op_sub = message_filters.Subscriber('/obj_position', PointCloud)  #published by objectdetector node
         self.obj_pub = rospy.Publisher('/map_objects', PointCloud , queue_size=1)
         self.ts = message_filters.ApproximateTimeSynchronizer([self.bb_sub,self.op_sub], 1, 1.5)
...
#                       44,45,46,47,48,49,54,55,64,65,67,70,73,75,76,79]
         ...
         if(len(self.objs)==0):
              ...
              for i in range(len(points)):  #check for similar objects
                   matched = 0
         ...
         cloud.header.frame_id = "rexrov/camera_link_optical"
         cloud.header.stamp = rospy.Time.now()
         cloud.points = geo_points
         cloud.channels = [classes]
         self.obj_pub.publish(cloud)
         with open('/home/nivnoetic/uuv_ws/src/maps/obj_map.csv', mode='w') as map1:  #store object pose and id in csv file
...
if __name__ == '__main__':
    path_file = '/home/nivnoetic/uuv_ws/src/maps/obj_map.csv'
